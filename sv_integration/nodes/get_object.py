...
# ---------------------------------------------------------------------------
def _debug(msg):
    print(f"[Hippo3D GetObject] {msg}")

def _debug_exc(msg):
    import traceback
    print(f"[Hippo3D GetObject ERROR] {msg}")
    traceback.print_exc()
...
def _is_hippo3d_occ_object(obj):
    ...
    Accepts the explicit kernel tag or any Hippo3D OCC custom property.
    """
    ...
    if obj.get("hippo_occ_preview") is True:
        return True
    # Fallback: any Hippo3D OCC-specific custom property indicates this is an OCC object.
    for key in obj.keys():
