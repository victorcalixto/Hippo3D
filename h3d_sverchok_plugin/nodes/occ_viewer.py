...
def _find_hippo3d_package_root():
    ...
    # 1. When the plugin is bundled inside Hippo3D.
    candidate = Path(__file__).resolve().parents[2]
    if (candidate / "kernels" / "occ_loader.py").exists():
        return candidate
    # 2. Known Blender module names for the main add-on.
    for ext_name in ["Hippo3D", "bl_ext.blender_development.Hippo3D"]:
        ...
        if mod is not None and hasattr(mod, "__file__"):
            ...
            if (candidate / "kernels" / "occ_loader.py").exists():
                return candidate
    # 3. Fallback: scan registered Blender add-ons.
    try:
        import bpy
        ...
        pass
    # 4. Final fallback: walk sys.path for a Hippo3D package.
    for p in sys.path:
        try:
...
def _surface_to_occ_shape(surface):
    ...
    # Ensure surface is properly wrapped (geomdl surfaces need this)
...
def _create_hippo_object(context, name, occ_shape_id, location, nurbs_shape_id=None, source_solid=None):
    ...
    try:
        ...
        if occ.has_shape(occ_shape_id):
            ...
            # Build a minimal mesh object and tag it like Hippo3D OCC objects.
...
def _bake_surface(context, name, surface, location=None):
    ...
    # Ensure surface is properly wrapped before baking.
    surface = _ensure_sverchok_surface(surface)
    if surface is None:
    ...
    def get_geometry(self):
        ...
        for params in zip_long_repeat(surfaces_s, res_u_s, res_v_s):
            for surface, ru, rv in zip_long_repeat(*params):
                ...
                # Ensure surfaces from upstream geomdl nodes are wrapped as proper SvNurbsSurface.
                surface = _ensure_sverchok_surface(surface)
                if surface is None:
                    continue
